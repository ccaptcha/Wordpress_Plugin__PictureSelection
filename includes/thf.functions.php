...
$error_code = 'thf_ccaptcha_ps_error';
 ...
 * @param bool $break
 */
function thf_ccaptcha_ps_display( $break = true ) {
 ...
 * Verify Captcha
 */
function thf_ccaptcha_ps_verify() {
	global $error_code;

	if ( ( $validate = thf_ccaptcha_ps()->CORE->verify() ) !== true ) {
		$error = new WP_Error();
...
# Login
/*-----------------------------------------------------------------------------------*/
function thf_ccaptcha_ps_form_login() {
	thf_ccaptcha_ps_display();
}

function thf_ccaptcha_ps_form_custom_login( $content = "" ) {
	ob_start();
	thf_ccaptcha_ps_display();
 ...
 */
function thf_ccaptcha_ps_check_login( $user ) {
	if ( !isset( $_POST['wp-submit'] ) ) {
		return $user;
	}

	$error = thf_ccaptcha_ps_verify();
...
# Register
/*-----------------------------------------------------------------------------------*/
function thf_ccaptcha_ps_form_register() {
	thf_ccaptcha_ps_display();
}
 ...
 */
function thf_ccaptcha_ps_form_wpmu_register( $errors ) {
	global $error_code;
...
function thf_ccaptcha_ps_check_register( $errors ) {
	$error = thf_ccaptcha_ps_verify();

	return is_wp_error( $error ) ? $error : $errors;
}

function thf_ccaptcha_ps_check_wpmu_register( $results ) {
	$error = thf_ccaptcha_ps_verify();
...
# Lost Password
/*-----------------------------------------------------------------------------------*/
function thf_ccaptcha_ps_form_lostpassword() {
	thf_ccaptcha_ps_display();
}

function thf_ccaptcha_ps_check_lostpassword( $allow ) {
	$error = thf_ccaptcha_ps_verify();
...
function thf_ccaptcha_ps_form_defaults( $defaults ) {
	ob_start();
	thf_ccaptcha_ps_display( false );
...
function thf_ccaptcha_ps_check_comment( $comment ) {
	...
	if ( is_wp_error( $error ) ) {
		wp_die( '<p> <strong>' . __( 'Error', THF_CCAPTCHA_PS_DOMAIN ) . ':</strong> ' . $error->get_error_message() . '</p>', $error->get_error_message(), [
			'back_link' => true,
		] );
	}
